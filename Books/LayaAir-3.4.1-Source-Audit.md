...
`settings/LayaSourceBaseline.json` 保存 22 个关键文件的 SHA256；`npm run check:engine-source` 会提取、规范化并逐项比对，避免只凭 `.d.ts` 或记忆设计框架。

## 已审计结论

- `Timer/EventDispatcher/Tween/Pool/Loader/Scene/LocalStorage/HttpRequest/SoundManager` 已提供完整基础能力，不建立同义管理器。
