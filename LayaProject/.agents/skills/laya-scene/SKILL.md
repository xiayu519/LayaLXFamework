...
3. World 专属场景用 `WorldScope.registerScene/openScene` 自动登记生命周期，后续使用返回的 route；根或显式管理操作可用 `lx.scenes`。不同 route 可并存，既有 SceneFlow/BaseGameScene 负责失效、Loading、失败保护和清理，见 [场景流程](../../../docs/scene-flow.md)。World 是上级生命周期 owner，SceneRegistry 保存实例，UI 归 Scene；不创建额外 UI 宿主或全局 current 猜测。原生独立 Scene 操作不强制经过切换事务。
