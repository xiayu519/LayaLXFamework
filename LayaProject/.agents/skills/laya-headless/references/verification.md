...
Windows 与 macOS 使用同一 npm 命令和 `~/.layaair` 版本注册结构。本机依赖由开发人员按环境文档准备，仓库只检测、不安装；工具通过 `LAYAAIR_INSTALL_DIR`、`LAYAAIR_IDE_HOME`、`BROWSER_PATH`、`PYTHON_PATH` 覆盖非标准位置。日常 push 只跑无 Laya CLI 的快速门禁；正式发布 Tag 或手动 release CI 才在双平台分别执行一次 `npm run verify:release`，不能共享或复制项目构建结果。
