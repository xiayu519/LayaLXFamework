...
Windows 与 macOS 使用同一 npm 命令和 `~/.layaair` 版本注册结构。工具通过 `LAYAAIR_INSTALL_DIR`、`LAYAAIR_IDE_HOME`、`BROWSER_PATH` 覆盖非标准安装位置；平台默认路径只能在对应系统分支中出现。双平台兼容必须由 CI 分别执行一次完整 `npm run verify`，共享静态检查可并行，但不能共享或复制项目构建结果。
