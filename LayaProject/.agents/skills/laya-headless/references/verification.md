...
Windows 与 macOS 使用同一 npm 命令和 `~/.layaair` 版本注册结构。本机依赖由开发人员按环境文档准备，仓库只检测、不安装；工具通过 `LAYAAIR_INSTALL_DIR`、`LAYAAIR_IDE_HOME`、`BROWSER_PATH`、`PYTHON_PATH` 覆盖非标准位置。运行时、资源与正式发布验收都在目标本机按需执行 `npm run verify:release`；GitHub Actions 只校验框架同步契约，不安装或检测这些本机依赖。
