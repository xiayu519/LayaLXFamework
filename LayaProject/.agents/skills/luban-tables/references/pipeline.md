...
- 生成：外层 `Design/genBin.bat` 与 `Design/genBin.command` 分别提供 Windows/macOS 双击入口，只转调既有 `npm run tables:generate`；输出位置由 `settings/GameProject.json` 指定，默认 sample 为 `src/game/sample/generated/tables/schema.ts`、`assets/bootstrap/game/tables/*.bin` 和路径稳定的 `.meta`。
