...
- 生成：外层 `Design/genBin.bat` 与 `Design/genBin.command` 分别提供 Windows/macOS 双击入口，只转调既有 `npm run tables:generate`；输出位置由 `settings/GameProject.json` 指定，模板默认把可调用表逻辑生成到 `src/game/logic/generated/tables/schema.ts`，数据输出到 `assets/bootstrap/tables/*.bin`，并保留路径稳定的 `.meta`。命名游戏需要独立表代码时再显式调整配置。
- 校验：`npm run tables:validate` 只验证源可生成；`npm run tables:check` 在系统临时目录重生成并逐字节对比，不复制 Laya 项目。
- 元数据：生成前保存目标中已有有效 `.meta`，替换内容后恢复其 UUID；资源移动后再次生成不会改变身份。新增文件才按目标路径生成确定性 UUID。非法 UUID 报错，不在清空目录后才处理。
