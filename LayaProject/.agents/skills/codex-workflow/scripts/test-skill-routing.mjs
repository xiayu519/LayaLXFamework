...
import { assertRoutingResult, buildRoutingSchema, loadRoutingEvaluation } from "./routing-evaluation.mjs";
...
const schemaTemplatePath = join(skillRoot, "evals", "routing-output.schema.json");
...
const schemaPath = join(evaluationRoot, "routing-output.schema.json");
const resultPath = join(evaluationRoot, "last-message.json");

try {
    const schemaTemplate = JSON.parse(readFileSync(schemaTemplatePath, "utf8"));
    writeFileSync(schemaPath, `${JSON.stringify(buildRoutingSchema(schemaTemplate, definition), undefined, 2)}\n`, "utf8");
    ...
    assertUsage(usage, settings.routing.inputTokens, settings.routing.outputTokens[policy.effort]);
