...
- 语义评测包含无需 Skill 的负例、邻域误触发和工作流决策；拒绝工具调用防止读取预期答案，分类评测不能冒充端到端代理执行证明。token 是执行后失败阈值，不是硬花费上限。只有语义输入变化才由开发者已登录的本地 Codex CLI 执行一次；普通 YAML、脚本、测试实现和文档改动只跑确定性门禁。GitHub CI 不调用模型或要求 API key。
...
- Windows/macOS 兼容结论必须来自发布 Tag 或手动触发的双平台 CI；每个平台原地构建一次并执行同一 Headless 验收，不能用日常 push 的静态门禁替代。
- Git 操作仅在开发者明确要求时执行。
