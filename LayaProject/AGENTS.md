...
项目按单人开发、单人维护定位。Codex 默认单代理；仅跨独立风险边界且有收益或用户要求时委派，隔离写入区域。模型默认仅在 `.codex/config.toml`，显式选择优先，Skill 不切换模型或强度；子代理继承，验收不降级。写前复读，保留已有改动；无法避开的同区域冲突停止报告。commit/push 等 Git 写操作需用户要求。
