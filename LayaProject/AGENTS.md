...
项目由单人开发维护。Codex 默认单代理；仅跨独立风险边界且有收益或用户要求时委派，隔离写入区域。项目不指定模型/强度；GPT-6 Sol `high/xhigh` 仅作推荐与评测，用户选择优先，Skill 不切换；子代理继承，验收不降级。写前复读，保留已有改动；无法避开的同区域冲突停止报告。commit/push 等 Git 写操作需用户要求。
