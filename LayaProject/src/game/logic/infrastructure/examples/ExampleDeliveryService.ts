...
export class ExampleDeliveryService extends Laya.EventDispatcher implements AppService, ExampleDeliveryState, ExampleDeliveryControls, ExampleInventoryCommands {
    public static readonly CHANGED = "example-delivery:changed";
    public readonly name = "example-delivery";
    private running = false;
    private pending = false;
    private message = "补给已就绪，打开背包查看。";
    private previousResponse: ExampleInventorySnapshot;

    public constructor(private readonly server: ExampleInventory, private readonly receiver: ExampleInventoryReceiver) {
        super();
    ...
    public get rewardPending(): boolean {
        return this.pending;
    }
    ...
    /** 明确标记为开发数据源，不代表已完成服务器连接或 IAP 权益同步。 */
    public async synchronize(signal: AbortSignal): Promise<void> {
        signal.throwIfAborted();
        if (!this.running) {
            throw new Error("Development delivery service is not running.");
        }
        ...
        if (this.receiver.applySnapshot(this.previousResponse) !== "applied") {
            throw new Error("Example login data was rejected.");
        }
    }
    ...
    public replayPreviousResponse(): void {
        if (this.running) {
            this.report(this.receiver.applySnapshot(this.previousResponse));
        }
    }
    ...
    private readonly deliverReward = (): void => {
        ...
        const patch = {
            version: this.server.version + 1, baseVersion: this.server.version,
    ...
    public use(id: string): boolean {
        ...
        if (this.server.applyPatch(patch) !== "applied") {
            return false;
        }
    ...
    private notify(message: string): void {
        ...
        this.event(ExampleDeliveryService.CHANGED);
    }
}
