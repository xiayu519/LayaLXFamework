...
/** 订阅归本次界面展示持有；模型保留自身生命周期，并通过原生事件通知。 */
export class UIBindings {
    private readonly bindings = new Set<DataBinding>();
    private active = true;
    private disposed = false;

    public constructor(private readonly view: Laya.Sprite, private readonly redDots?: RedDotStore) {
    }

    /** 立即渲染当前快照，再将原生通知合并到 callLater 执行。 */
    public bindData(source: Laya.EventDispatcher, event: string | readonly string[], render: () => void): () => void {
        this.requireAlive();
        const events = [...new Set(typeof event === "string" ? [event] : event)];
        if (!events.length || events.some(key => !key)) {
            throw new Error("UI binding events must be non-empty.");
        }
    ...
    /** 多个红点可独立监听同一路径，包括复用的 GList 行。 */
    public bindRedDot(badge: Laya.Sprite, key: string, options: RedDotOptions = {}): () => void {
        this.requireAlive();
        const store = this.redDots;
        if (!store || store.disposed) {
            throw new Error("UI red dot bindings require an active RedDotStore.");
        }
        store.get(key); // 添加原生订阅前先校验。
    ...
    private activate(binding: DataBinding): void {
        if (!this.active || !this.bindings.has(binding) || binding.active) {
            return;
        }
        binding.active = true;
    ...
    private suspend(binding: DataBinding): void {
        ...
        if (binding.pending) {
            binding.pending = false;
            try {
                Laya.timer.clearCallLater(binding, binding.refresh);
            } catch (error) {
                errors.push(error);
            }
        }
        for (const event of binding.events) {
            try {
                binding.source.off(event, binding, binding.notify);
            } catch (error) {
                errors.push(error);
            }
        }
    ...
    private requireAlive(): void {
        if (this.disposed || this.view.destroyed) {
            throw new Error("The UI binding presentation has ended.");
        }
    }
}
