...
vi.stubGlobal("Laya", {
    ...
    Sprite: FakeWidget, Script: class {}, UIConfig2: { modalLayerColor: "#00000088" }, GWidget: FakeWidget, GWindow: FakeWindow, GRoot: { inst: groot }, Prefab: FakePrefab,
...
class TestView extends Laya.GWidget { value = ""; }
...
beforeEach(() => {
    timer.reset();
    load.mockReset().mockImplementation(() => Promise.resolve(new FakePrefab(() => new TestView())));
...
describe("SceneUI", () => {
    it("evicts only closed cached views, leaving live and pending presentations intact", async () => {
        ...
        const route = router.registerView({ ...viewRoute("cached"), retention: "hide" as const });
    ...
    it("reuses hidden views and windows within the scene but destroys both on exit", async () => {
        const { router } = createRouter();
        const cleaned = vi.fn();
        const page = router.registerView({ ...viewRoute("page", (view, args, session) => {
            view.value = args;
            session.lifetime.defer(cleaned);
        }), retention: "hide" });
        const popup = router.registerView({ ...popupRoute("confirm"), retention: "hide" });
    ...
    it("cleans both visible and retained layout subscriptions when the scope is disposed", async () => {
        const { router, layout } = createRouter();
        const route = router.registerView({ ...viewRoute("page"), retention: "hide" });
        const scene = router.createSceneUI(new TestView());
    ...
    it("pauses covered Screen pages and restores their state while leaving HUD active", async () => {
        ...
        const hud = router.registerView({ ...viewRoute("hud"), layer: UILayer.HUD });
        const root = new TestView();
        const scene = router.createSceneUI(root);
    ...
    it("ignores mask input for a disposed scope whose failed window still awaits cleanup", async () => {
        ...
        const { router } = createRouter();
        const route = router.registerView({ ...popupRoute("failed"), viewType: RetryWindow });
    ...
    it("releases scope registries, retained windows and layout listeners across repeated scene cycles", async () => {
        const { router, layout } = createRouter();
        const page = router.registerView({ ...viewRoute("page"), retention: "hide" });
        const popup = router.registerView({ ...popupRoute("popup"), retention: "hide" });
...
describe("SceneUI owner and cleanup boundaries", () => {
    it("cancels child loading when its parent closes and rejects the old session after reopen", async () => {
        ...
        const parent = router.registerView({ ...viewRoute("parent", (_view, _args, session) => { sessions.push(session); }), retention: "hide" });
    ...
    it("reuses singleton children within one owner and isolates the same route across owners", async () => {
        ...
        const parent = router.registerView({ ...viewRoute("parent", (_view, args, session) => { sessions.set(args, session); }), multiplicity: "multiple" });
        const child = router.registerView(popupRoute("child"));
        const scene = router.createSceneUI(new TestView());
    ...
    it("keeps bindings paused when another view is shown under an inactive scene root", async () => {
        ...
        const hud = router.registerView({ ...viewRoute("hud-paused"), layer: UILayer.HUD });
        const scene = router.createSceneUI(new TestView());
    ...
    it("notifies onClosed once per displayed presentation across hide, reopen and native destroy", async () => {
        const { router } = createRouter();
        const closed = vi.fn();
        const route = router.registerView({ ...viewRoute("page"), retention: "hide", onClosed: closed });
        const scene = router.createSceneUI(new TestView());
    ...
    it("closes every matching instance even when one presentation cleanup fails", async () => {
        const { router } = createRouter();
        const route = router.registerView({ ...viewRoute("multiple", (_view, args, session) => {
    ...
    it("retries a native destruction failure on a repeated close without retaining a successful retry", async () => {
        ...
        const { router } = createRouter();
        const route = router.registerView({ ...viewRoute("retry"), viewType: RetryView });
    ...
    it("restores the lower page and removes the mask after a modal page closes with a cleanup error", async () => {
        ...
        const upperRoute = router.registerView({ ...viewRoute("bad-modal-page", (_view, _args, session) => {
            session.lifetime.defer(() => { throw new Error("cleanup failed"); });
        }), modal: true });
        const scene = router.createSceneUI(new TestView());
    ...
    it("retains diagnostics when native destroy returns without destroying, then permits a real retry", async () => {
        ...
        const { router } = createRouter();
        const route = router.registerView<string, NoopView>({ ...viewRoute("noop"), viewType: NoopView });
...
function viewRoute(id: string, bind: (view: TestView, args: string, session: UIViewSession) => void | Promise<void>
    ...
    return { id, url: `ui/${id}.lh`, viewType: TestView, bind };
}

function popupRoute(id: string): UIViewRoute<string, TestView> {
    return { id, url: `ui/${id}.lh`, multiplicity: "singleton", retention: "destroy", layer: UILayer.Popup,
        layout: "fullscreen", modal: true, closeOnMaskClick: true, viewType: TestView, navigation: "overlay", bind: (view, args) => { view.value = args; } };
}
