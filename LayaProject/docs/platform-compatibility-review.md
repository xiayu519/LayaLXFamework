...
2026-09-14，基于 `4afd361124e61687417a17326fec71ebadedd93b` 审查，随后按用户“发现了问题就修复并且验证”实施。已修复取消 API 缺失导致启动或退出失败、未支持平台误选 Web、视口零值与未知安全区混淆三类问题，并完成单测、依赖检查及实际 Laya 构建的专项验证。该次平台审查按用户要求将支付保留为 TODO；后续单独实施的通用支付流程见 [支付设计](payment-design.md)，不属于下列历史平台验证范围。
...
- TODO：真实支付 SDK、订单服务与商店验收。该次审查时使用 UnsupportedPurchasePlatform，后续由未配置时明确拒绝的 PurchaseModule 替代，接入与专项证据见 [支付设计](payment-design.md)。
