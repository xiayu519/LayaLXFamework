...
| 模板 | 路径 | 结构与用途 |
| --- | --- | --- |
| Fullscreen | [Fullscreen.lh](../assets/bootstrap/framework/ui/templates/Fullscreen.lh) | Root 全屏；full + safeContent(top/full/mid/bottom)。主界面按四区拼装，其他全屏页省略不用的槽位。route.layout = fullscreen。 |
| Popup | [Popup.lh](../assets/bootstrap/framework/ui/templates/Popup.lh) | Root 全屏；safeContent/mid 为 560×390 的内容画布，可按需求改设计尺寸。标题、按钮、列表都放 mid。route.layout = center-popup；框架只动画 mid。 |
...
骨架不绑定业务脚本、不创建业务窗口路由。复制到当前游戏后分配新资产 UUID，设置该游戏 Runtime，并通过已有 UIRouter 注册。Fullscreen 的空 top/bottom 初始高度为 0，填入内容时设置设计高度。默认 Popup 的 modal 遮罩由 GRoot 管理，closeOnMaskClick 默认 true；设置 false 可保留遮挡并禁止空白关闭。modal:false 可在 full 内使用自定义全屏按钮，细节见 [Mask 与关闭后处理](ui-layout.md#通用-mask-与关闭后处理)。
...
2026-09-10 本机验证：typecheck、62 项相关单测、架构/资源布局检查和 15 个层级资产的官方解析通过；6 个 Skill 路由案例及独立模板组合执行检查通过。LayaAir 3.4.1 原地构建、100 次 UI/Pool 循环、8 种浏览器分辨率 × 4 组安全区均通过。背包列表随剩余空间实例化 6–13 个显示项，行高始终为 88、标题字号为 22；通用列表和三列网格分别为 7、15 个显示项。覆盖 Mask 堆叠/关闭开关、mid 动画、关闭钩子、原生池回收及动画期间 resize，无 404、运行时错误或停机 owner 残留。

另通过 CDP 连续切换实际 CSS 视口 390×844 → 600×800 → 320×568 → 390×844，并检查全屏和弹窗截图。以上为 Windows Headless Chromium / SwiftShader 结果；刘海与胶囊数据为模拟输入，未代替小游戏真机、macOS 或目标 GPU 验收。
