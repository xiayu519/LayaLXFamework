...
- [DynamicImage.lh](../assets/bootstrap/framework/ui/components/DynamicImage.lh)：GLoader Runtime 组件，运行时换图使用 src，适用于嵌套列表图标、按钮 icon、九宫格和图集动画，见 [资源生命周期](ui-resource-lifecycle.md)。
...
- [FullscreenMid.lh](../assets/bootstrap/game/ui/examples/FullscreenMid.lh)：全屏 mid 示例，中央面板复用 PanelChrome。由背包中的“居中展示”按钮打开，展示居中、缩小恢复与按钮交互。

主界面 = 全屏骨架 + 资源栏/内容/导航 + 散图；弹窗背包 = 弹窗骨架 + 物品网格 + 详情/操作 + 散图。标题栏、按钮或行组件不需要重复增加 full/mid。

当前示例使用原生 Runtime 与展示绑定函数，节点字段交给 IDE 生成：

| 示例 | 原生节点引用 | 展示行为 |
| --- | --- | --- |
| FrameworkStatus | [FrameworkStatusView](../src/game/logic/presentation/ui/FrameworkStatusView.ts) | [FrameworkStatusPage](../src/game/logic/presentation/ui/FrameworkStatusPage.ts) |
| Inventory | [ExampleInventoryView](../src/game/logic/presentation/ui/examples/ExampleInventoryView.ts) | [ExampleInventoryPage](../src/game/logic/presentation/ui/examples/ExampleInventoryPage.ts) |
| FullscreenMid | [ExampleFullscreenMidView](../src/game/logic/presentation/ui/examples/ExampleFullscreenMidView.ts) | [ExampleFullscreenMidPage](../src/game/logic/presentation/ui/examples/ExampleFullscreenMidPage.ts) |
| Confirmation | [ExampleConfirmationView](../src/game/logic/presentation/ui/examples/ExampleConfirmationView.ts) | [ExampleConfirmationPage](../src/game/logic/presentation/ui/examples/ExampleConfirmationPage.ts)，由打开它的展示拥有 |
