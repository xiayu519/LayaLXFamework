...
UI route 还声明 `layout`：非 Popup 层默认 `fullscreen`，Popup 层默认 `center-popup`，也可显式使用 `safe-screen`。所有窗口使用全屏 Root 下 full + safeContent(top/full/mid/bottom)，不用的槽位省略或高度为 0。`center-popup` 由框架只对 safeContent/mid 播放缩放开合动画，全屏布局不播放；关闭/销毁会取消旧 Tween，并以版本令牌隔离晚到回调。`UILayoutService` 只以运行时 `GRoot` 的实际逻辑宽高为布局边界，并把平台窗口坐标转换为 Stage 坐标；项目可自由选择自己的设计分辨率，`.lh` 的创作画布尺寸不是运行时固定边界。完整约定见 [ui-layout.md](ui-layout.md)。
