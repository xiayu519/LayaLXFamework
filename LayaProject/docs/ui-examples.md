...
- [Inventory.lh](../assets/bootstrap/game/ui/examples/Inventory.lh)：完整声明 `full + safeContent(top / full / bottom)`；标题和汇总归 top，虚拟列表归 safeContent/full，选择信息和操作按钮归 bottom。列表随上下区域和安全区拉伸，行高与字号不变；源资产声明 Scroller、模板节点及宽高 Relation。
...
PanelChrome 是装饰子组件，不是全屏或弹窗骨架。全屏窗框位于 safeContent/top，弹窗窗框位于 safeContent/mid，两者均在窗口构造时用 `this.closeButton = pane.frame.getChild("closeButton")` 绑定关闭按钮。固定节点始终来自 `.lh`，无需运行时补建。底部按钮相对 `actions` 同时按比例关联宽度和左坐标，侧边安全区收窄时保持间距。
